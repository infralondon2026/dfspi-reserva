...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [storedLocale, setStoredLocale] = usePersisted<Locale>(
    "dfspi-locale",
    "es",
  );
  const firstSegment = location.pathname.split("/").filter(Boolean)[0];
  const routeLocale: Locale | undefined = ["es", "pt", "en"].includes(
    firstSegment,
  )
  ...
  useEffect(() => {
    if (routeLocale && routeLocale !== storedLocale)
  ...
  const setLocale = useCallback(
    (next: Locale) => {
      ...
      if (parts.length && ["es", "pt", "en"].includes(parts[0]))
        parts[0] = next;
  ...
  const update = useCallback(
    (productId: string, quantity: number) => {
      setCart((current) => {
        ...
        return current.map((i) =>
          i.productId === productId
            ? { ...i, quantity: Math.min(quantity, stock) }
            : i,
        );
      });
    },
...
export const useLocale = () =>
  useRequired(useContext(LocaleContext), "useLocale");
export const useStoreData = () =>
  useRequired(useContext(StoreContext), "useStoreData");
