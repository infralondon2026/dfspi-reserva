...
const productTitle = (brand: string, name: string) =>
  name.toLocaleLowerCase().startsWith(brand.toLocaleLowerCase())
    ? name
    : `${brand} ${name}`;
...
export default function Seo() {
  ...
  useEffect(() => {
    ...
    const product =
      route[0] === "producto"
        ? products.find(
            (item) => item.id === route[1] || item.slug === route[1],
          )
        : undefined;
    const title = product
      ? `${productTitle(product.brand, product.name)} | Duty Free Shop Puerto Iguazú`
      : route[0] === "catalogo"
        ? copy.catalog
        : route[0] === "legales"
          ? copy.legal
    ...
    const description = product
      ...
      : copy.description;
    const canonicalPath =
      location.pathname.endsWith("/") || route.length
        ? location.pathname
        : `${location.pathname}/`;
    ...
    const graph: Record<string, unknown>[] = [
      ...
      },
      {
        ...
        paymentAccepted:
          "Visa, Mastercard, American Express, Visa Electron, Maestro, PIX, Cash",
    ...
    if (route[0] === "catalogo") {
      graph.push({
        "@type": "ItemList",
        "@id": `${SITE_URL}/${locale}/catalogo/#products`,
        name: copy.catalog,
        numberOfItems: products.length,
        itemListElement: products.map((item, index) => ({
          "@type": "ListItem",
          position: index + 1,
          url: `${SITE_URL}/${locale}/producto/${item.slug || item.id}/`,
          name: productTitle(item.brand, item.name),
        })),
      });
    }
    ...
    if (product) {
      const image = product.image.startsWith("http")
        ? product.image
        : `${SITE_URL}/${product.image.replace(/^\//, "")}`;
      graph.push({
        "@type": "Product",
  ...
  }, [locale, location.pathname, products]);
