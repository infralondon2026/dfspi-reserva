...
export default function Chat() {
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    ...
    return [
      { from: "bot", text: initial.text, suggestions: initial.suggestions },
    ];
  });
      ...
      {open && (
        <aside className="chat" aria-label={tr("chatAssistant")}>
          ...
          <div className="chat-body" aria-live="polite">
            ...
            <button
              className="chat-topics-toggle"
              type="button"
              onClick={() => setShowTopics((current) => !current)}
            >
              <List size={15} /> {tr("chatTopics")}{" "}
              <ChevronDown className={showTopics ? "open" : ""} size={15} />
            </button>
            ...
            {showTopics && (
              <div className="chat-topics">
                {faqs.map((faq) => (
                  <button
                    key={faq.id}
                    onClick={() => reply(faq.question[locale])}
                  >
                    {faq.question[locale]}
                  </button>
                ))}
