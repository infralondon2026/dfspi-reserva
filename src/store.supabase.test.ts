...
function mockClient(overrides: {
  ...
  const invoke = overrides.invoke ?? vi.fn(async () => ({ data: { ok: true }, error: null }))
...
describe('supabase adapter', () => {
  it('maps products/variants/inventory rows into the frontend Product shape', async () => {
    const { client } = mockClient({ tables: { products: { data: [productRow], error: null } } })
  ...
  it('skips products without an active variant', async () => {
    const row = { ...productRow, variants: [{ id: 'v', active: false, inventory: null }] }
    const { client } = mockClient({ tables: { products: { data: [row], error: null } } })
  ...
  it('creates reservations through the create_reservation RPC and sends the email', async () => {
    ...
    expect(rpc).toHaveBeenCalledWith('create_reservation', {
      customer: { name: 'Ada Lovelace', email: 'ada@example.com', phone: '12345678' },
    ...
    expect(invoke).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        body: {
          reservation: expect.objectContaining({ code: 'IGZ-AB12-CD34', email: 'ada@example.com' }),
        },
      }),
    )
  })
  ...
  it('still returns the reservation when the email invocation fails', async () => {
    ...
    const { client } = mockClient({
      invoke,
      rpc: {
        data: { reservationCode: 'IGZ-EE11-FF22', status: 'confirmada', expiresAt: 'x', total: 10 },
        error: null,
      },
    })
  ...
  it('looks reservations up by code+email via get_reservation and maps items', async () => {
    ...
    const found = await createSupabaseAdapter(client).findReservation('igz-ab12-cd34 ', ' ADA@example.com')
  ...
  it('returns null when the reservation is not found', async () => {
    ...
    expect(await createSupabaseAdapter(client).findReservation('IGZ-XXXX-XXXX', 'a@b.com')).toBeNull()
  })
  ...
  it('rejects sign-in for users without an active admin_users row', async () => {
    ...
    await expect(createSupabaseAdapter(client).signIn('user@example.com', 'secret')).rejects.toThrow(
      'not_admin',
    )
  })
  ...
  it('signs an admin in and returns their role', async () => {
    const { client } = mockClient({
      tables: { admin_users: { data: { role: 'operations', active: true }, error: null } },
      auth: {
        signInWithPassword: vi.fn(async () => ({
  ...
  it('updates reservation status through release_reservation_stock', async () => {
    ...
    await createSupabaseAdapter(client).setReservationStatus('IGZ-AB12-CD34', 'cancelada')
