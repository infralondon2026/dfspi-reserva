...
import { products as approvedProducts } from "./data";
...
const PRODUCT_SELECT =
  "id,sku,brand,name,subtitle_es,subtitle_pt,description_es,description_pt,image_url,price_usd,original_price_usd,featured," +
...
function mapProduct(row: ProductRow): Product | null {
  ...
  const approved = approvedProducts.find(
    (product) =>
      product.brand.toLocaleLowerCase() === row.brand.toLocaleLowerCase() &&
      product.name.toLocaleLowerCase() === row.name.toLocaleLowerCase(),
  );
  return {
    id: variant.id,
    slug:
    ...
    subtitle: {
      es: row.subtitle_es,
      pt: row.subtitle_pt,
      en: approved?.subtitle.en || row.subtitle_es,
    },
    category,
    image: row.image_url,
    ...
    description: {
      ...
      en: approved?.description.en || row.description_es,
    },
  };
}
